<script lang="ts">
	...
	$: ratingClass = ratingStyles[firm.economicRating] || {
		bg: 'bg-gray-300',
		text: 'text-gray-500'
	};
</script>
...
<div
	class="flex h-fit w-xs min-w-xs cursor-pointer flex-col gap-5 bg-white p-7 sm:w-full md:w-sm lg:w-full"
>
	<div class="flex items-center justify-start gap-7">
		<!-- Rating circle with dynamic classes -->
		<div
			class="flex h-12 w-12 items-center justify-center rounded-full text-center text-xl font-bold {ratingClass.bg} {ratingClass.text}"
		>
			{firm.economicRating}
		</div>
	...
	<div class="flex items-center justify-between gap-2.5 text-sm font-semibold text-amber-900">
		<span class="tag w-full border-amber-300 bg-amber-100">{firm.taxes}</span>
		<span class="tag w-full border-amber-300 bg-amber-100">{firm.debt}</span>
		<span class="tag w-full border-amber-300 bg-amber-100">{firm.activity}</span>
	</div>
